...
class Clickwrap:
    ...
    def create(args, request):
        ...
        TPL_PATH = Path(__file__).parent.parent.parent / 'templates' / file_name            
        ...
        ds_client = DsClient.get_configured_instance(
            ...
            config.CLICKWRAP_BASE_HOST
        )

        uri = f"{config.CLICKWRAP_BASE_URI}/{account_id}/clickwraps"
        ...
        uri = f"{config.CLICKWRAP_BASE_URI}/{account_id}/clickwraps/{clickwrap_id}/versions/1"
    ...
    @staticmethod
    def get_user_agreements(args, request):
        ...
        ds_client = DsClient.get_configured_instance(access_token, config.CLICKWRAP_BASE_HOST)

        uri = f"{config.CLICKWRAP_BASE_URI}/{account_id}/clickwraps/{args['clickwrap_id']}/users"
        response = ds_client.call_api(
        ...
        if user_agreements:
            for agreement in user_agreements:
                ...
                if time_delta <= config.CLICKWRAP_TIME_DELTA_IN_MINUTES:
